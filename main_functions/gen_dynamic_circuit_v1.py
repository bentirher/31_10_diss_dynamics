...
def get_circuit(n:int, omega_m:list, omega_c:list, g:list, gamma:list, kappa:list, initial_state:list, r:int) -> QuantumCircuit:
    ...
    for l in range(r):
        ...
        # Now we apply Ps to the first layer and do the classical check
        ...
        for j in range(0, n-1, 2):
            ...
            with else_:
                ...
                qc.cry(theta_minus_e[j], system[j], environment[j])
                qc.cry(theta_plus_e[j], system[j+1], environment[j+1])
                qc.cx(environment[j], system[j])
                qc.cx(environment[j+1], system[j+1])
