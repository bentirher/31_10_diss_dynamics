...
def get_circuit(n:int, omega_m:list, omega_c:list, g:list, gamma:list, kappa:list, initial_state:list, r:int, backend:BackendV2) -> QuantumCircuit:
    ...
    num_labels = ((2**3)*(n-1)*r)*(n + math.trunc(n/2)) # at least two qubits are involved in each conditional block, the rest are idle.
    ...
    for l in range(r):
        ...
        for j in range(0, n-1, 2):
            ...
            used_qbits.append(system[j])
            used_qbits.append(system[j+1])
        ...
        for j in range(0, n-1, 2):
            ...
            used_qbits.append(system[j])
            used_qbits.append(system[j+1])
            used_qbits.append(environment[counter])
