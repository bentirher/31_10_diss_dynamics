...
def get_circuit(n:int, omega_m:list, omega_c:list, g:list, gamma:list, kappa:list, initial_state:list, r:int, backend:Union[str, BackendV2]) -> QuantumCircuit:
    ...
    if all(x == omega_m[0] for x in omega_m): # Have to set this for identical emitters,
        # otherwise the calculation of sin and cos yield error.

        sen_theta = [1/np.sqrt(2) for i in range(n-1)]
        cos_theta = sen_theta
    ...
    for l in range(r):
        ...
        # First layer of the interaction and the decay
        ...
        if n > 2:
            ...
            for j in range(1, n-1, 2):
                ...
                with else_:
                    ...
                    qc.x(environment[counter]) # It turned into 1, we have to make it go back to 0.
                    qc.cry(theta_minus_e[j], system[j], environment[counter])
                    qc.cx(environment[counter], system[j])
                    qc.reset(environment[counter])
                    qc.cry(theta_plus_e[j], system[j+1], environment[counter])
